public class Constants {
    public static final int SIMULATION_REPETITION_TIMES = 10;
    public static final long RANDOM_SEED = 1234567892;
    public static final double b = 0.9;
    public static final double PARTICLE_MAX_RADIUS = 0.3;
    public static final double PARTICLE_MIN_RADIUS = 0.1;
    public static final double CIRCLE_RADIUS = 11;
    public static final double HUMAN_DESIRED_VELOCITY = 4;
    public static final double INITIAL_MIN_DISTANCE_TO_ZOMBIE = 1.;
    public static final double ZOMBIE_SEARCH_SPEED = 0.3;
    public static final double ZOMBIE_SEARCH_RADIUS = 4.;
    public static final double HUMAN_SEARCH_RADIUS = 4;
    public static final double MAX_TIME = 300.;
    public static final double INFECTION_DURATION = 7.;
    public static final double WANDER_TARGET_DISTANCE_EPSILON = 0.1;
    public static final double WANDER_TARGET_TIME = 8;
    public static final double EXPANSION_TIME = 0.5;
}
